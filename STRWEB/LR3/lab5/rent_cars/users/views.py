...
from cars.models import BodyType, Car, Discounts, Penalties, PromoCode, Rental, Cart
from users.models import Customer, User, Partner, SliderSettings
from django.contrib.auth.decorators import user_passes_test
from datetime import timedelta, date
from django.core.exceptions import ObjectDoesNotExist
from django.utils import timezone
import calendar
from users.forms import SliderSettingsForm
...
from portal.models import NewsArticle
...
def index(request):
    ...
    article = NewsArticle.objects.order_by('time_create').last()
    partners = Partner.objects.all()

    settings = SliderSettings.objects.first()
    formSlider = SliderSettingsForm(instance=settings)
    ...
    if request.user.is_authenticated:
        ...
        # Генерация HTML-календаря с выделением текущего дня
        ...
        cal = CustomHTMLCalendar().formatmonth(current_year, month_number)
            ...
            session = stripe.checkout.Session.create(
                ...
                line_items=[{
                    'price_data': {
                        ...
                        'unit_amount': int(rental.total_amount*100), 
                    },
                    'quantity': 1,
                }],
                mode='payment',
...
def payment_success(request):
    ...
    if session_id:
        ...
        # Найдите аренду, связанную с этой сессией (если у вас есть поле для хранения session_id)
        # rental = Rental.objects.get(session_id=session_id)
        
        # Примерно здесь вы можете отметить аренду как завершенную и удалить объект из корзины
        # cart_item.delete()
...
def all_items(request):
    # Фильтруем активные и неактивные объекты для каждой модели
    active_penalties = Penalties.objects.filter(active=True)
    inactive_penalties = Penalties.objects.filter(active=False)

    active_promo_codes = PromoCode.objects.filter(active=True)
    inactive_promo_codes = PromoCode.objects.filter(active=False)

    active_discounts = Discounts.objects.filter(active=True)
    inactive_discounts = Discounts.objects.filter(active=False)

    logging.info("Вызвана страница всех бонусов")

    context = {
        'active_penalties': active_penalties,
        'inactive_penalties': inactive_penalties,
        'active_promo_codes': active_promo_codes,
        'inactive_promo_codes': inactive_promo_codes,
        'active_discounts': active_discounts,
        'inactive_discounts': inactive_discounts,
    }

    return render(request, 'users/all_items.html', context)
...
def user_rentals(request):
    ...
    return render(request, 'users/profile.html', {'rentals': rentals, 'carts': carts})
...
def task7_prot(request):
    return render(request, 'users/task7_prot.html')

def task7_class(request):
    return render(request, 'users/task7_class.html')
...
def remove_from_cart_view(request, item_id):
    if request.method == 'POST':
        ...
        cart_item = get_object_or_404(Cart, id=item_id, user__user=request.user)
        cart_item.delete() 
        return redirect('user_rentals')  
    return redirect('user_rentals')


def order_car_view(request, car_id):
    car = get_object_or_404(Car, id=car_id)
    customer = Customer.objects.get(user=request.user)
    cart_item = Cart.objects.filter(car=car, user=customer).first()  # Ищем объект корзины с выбранной машиной
...
def rentals_per_model(request):
    rentals_data = (
        Rental.objects
        .values('car__model__name')
        .annotate(rental_count=Count('id'))
        .order_by('car__model__name')
    )
    data = {
        'labels': [item['car__model__name'] for item in rentals_data],
        'rentalCounts': [item['rental_count'] for item in rentals_data]
    }
